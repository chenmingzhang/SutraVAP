...
!       R1EFF   =  -1.469D-5*AA*(AAPVNN-BAPVN)
	   R1EFF   = -1.469D-5*AA*(AA*PSIM)**(VN-1)*(  (1+ (AA*PSIM)**-VN)**(1-1/VN) -1)
...
!new
	   R_2_M = R1EFF + R_C_M
	   IF (R_M_AY.LT.R1EFF)THEN
		 R_M_AY=R1EFF
	   ENDIF	
	   THICKNESS_FUNNEL= R_PARTICLE/EXP(1/R_C_M*(R_M_AY-R1EFF))
!      (E)FFECTIVE (S)ATURATION AFTER G FUNCTION
       SLENSL=AAPVN**(-VNF)
       SET = SLENSL**(1.D0+5.D-1)
	   ETI=0.66D0*(TAL+THICKNESS_FUNNEL)*POR*DLOG(5.D4)/(5D-2*DLOG(PSIC+5.D0))
!      POR*SET
!      PORSE=POR*SET
!new
...
!       SURFRSIS=TAL/DVA(TSK)*(1/RELKV-1)  	
!      WRITE(66,1001)
!	  1001 FORMAT(7X,'PP', 7X, 'PSIM', 7X, 'SLENSL', 7X, 'SURFRSIS')
   ENDIF ! MSR
	   WRITE(100,1001) PP, PSIM, SLENSL, SURFRSIS
!	   WRITE(200,1002) ETI, EVAPO_CAPILLARY, R1EFF, RELKV, R_3_AY	   
	   1001 FORMAT(4E15.6,",")
!	   1002 FORMAT(5E15.6,",")
!	   1002 FORMAT(E15.6,",",E15.6,",",E15.6,",",E15.6)  
