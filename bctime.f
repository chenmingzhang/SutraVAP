...
C      MVT IS A SWICH FOR OPEN (=1),OR CLOSE THE VAPOR TRANSPORT
  ...
  !FIND OUT HORIZONTAL OR VERTICAL NODE J=1 HORIZONTAL J=2 VERTICAL
      DO 185 J=1,2             
        CALL NNODE(IIQP,J,IQP,NDPT,NREF,NSOPI)
   ! THE NEXT NODE SHOULD BE EXIST
        IF (IIQP.NE.0.) THEN                                           
          II=IQSOP(IIQP)
   ! THE NEXT NODE SHOULD NOT BE SATURATED
...
C    FOR DEBUGGING 
      IF (SW(IABS(II)).LE.0.05.AND.J.EQ.2)THEN
       DD=1.D0
      ENDIF
C     VAPOR CALCULATION
