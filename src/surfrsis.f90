...
!	   R_AVG = -ZETA*AA*(-AA*PSICB)**(VN-1)*((1+(-AA*PSICB)**-VN)**(1-1/VN)-1)
...
!new
	   ...
	   IF (R_M_AY.LT.R_0_AY)THEN
		 R_M_AY=R_0_AY
	   ENDIF	
	   THICKNESS_FUNNEL= R_PARTICLE/EXP((R_M_AY/R_0_AY-1)
!      (E)FFECTIVE (S)ATURATION AFTER G FUNCTION
       SLETSL=(1+ (-AA*PSIM)**VN)**(1/VN-1)
	   THICKNESS_DRY = (2*YY*DLOG(PSIC+PSIP))/DLOG(PSIC0)
	   ETI= COER/( THICKNESS_DRY/(0.66D0*TAL*POR) + ETR )
!new
!      GET SATURATION FROM EFECTIVE SATURATION
       BETA = (DLOG(PSIC0)-DLOG(PSIC))/DLOG(PSIC0)
	   SATURATION_LIQUID = (1-BETA*SWRES)*SLETSL +BETA*SWRES
	   RELATIVE_WETTED_SURFACE=SLETSL*(1-THICKNESS_FUNNEL*(1-POR)/R_PARTICLE)
!	   RELATIVE_WETTED_SURFACE=SLETSL*POR
	   IF (RELATIVE_WETTED_SURFACE.GT.1.D0)THEN
	     RELATIVE_WETTED_SURFACE=1.0D0
	   ENDIF
	   R_3_AY=R_M_AY/RELATIVE_WETTED_SURFACE**0.5D0
	   IF (R_3_AY.LT.R_2_AY)THEN
		 R_3_AY=R_2_AY
	   ENDIF	
	   EVAPO_CAPILLARY=1/(1+R_3_AY**2*THICKNESS_FUNNEL/R_2_AY**2/TAL+R_M_AY/2/TAL/RELATIVE_WETTED_SURFACE*&
...
!	   RELATIVE_WETTED_SURFACE=SET*POR
!	   RELATIVE_WETTED_SURFACE=SET*(R_M_AY**2/R_2_AY**2)
	   IF (RELATIVE_WETTED_SURFACE.GT.1.D0)THEN
	     RELATIVE_WETTED_SURFACE=1.0D0
	   ENDIF	   
	   R_3_AY=R_M_AY/RELATIVE_WETTED_SURFACE**0.5D0
	   ...
	   EVAPO_CAPILLARY=1/(1+R_3_AY**2*THICKNESS_FUNNEL/R_2_AY**2/TAL+R_M_AY/2/TAL/RELATIVE_WETTED_SURFACE*&
         (2*FREE_PATH_GAS_M/R_M_AY+1/(1+FREE_PATH_GAS_M/R_M_AY)-RELATIVE_WETTED_SURFACE**0.5))
!	   EVAPO_CAPILLARY=1/(1+R_3_AY**2*0/R_2_AY**2/TAL+R_M_AY/2/TAL/RELATIVE_WETTED_SURFACE*&
!         (2*FREE_PATH_GAS_M/R_M_AY+1/(1+FREE_PATH_GAS_M/R_M_AY)-RELATIVE_WETTED_SURFACE**0.5))
	   RELKV=EVAPO_CAPILLARY*(1-ETI)+ETI
!	   SURFRSIS=1/RELKV
       SURFRSIS=TAL/DVA(TSK)*(1/RELKV-1)
...
!	   RELATIVE_WETTED_SURFACE=SET*(1-THICKNESS_FUNNEL*(1-POR)/R_PARTICLE)
   ...
   ENDIF ! MSR
	   ...
	   WRITE(200,1001) R_3_AY, EVAPO_CAPILLARY, R_M_AY, RELKV  
	   1001 FORMAT(4E15.6,",") 
