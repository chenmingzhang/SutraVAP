...
C  2. THE UNIT OF ALPHA IS [M-1] RATHER THAN [MS2KG-1]
     ...
     1   SWRES4,DLAM4,PSICB4,PSIC0,ECTO
...
C  (POSITIVE) 
      PSIC=-PRES/9.8D3
      SI=SWRES*DLOG(PSIC0/PSIC)/DLOG(PSIC0)
      SWRMS1=1.D0-SI
...
C      DNUM=AA*(VN-1.D0)*SWRMS1*(AA*(-PRES)/9.8D3)**(VN-1.D0)/9.8D3
      DNUM=AA*(1.D0-VN)*SWRMS1*(AA*PSIC)**(VN-1.D0)
...
C    SEE RELATIVEK.SAGE FOR REFERENCE
      SWSTAR = 1.D0/AAPVNN
      RELK   =   DBLE (SWSTAR**ECTO*                                     UNSAT........12400
...
C         TERM1 IS NOT NEEDED SEE PAGE 170
          ...
          DLPSICB=DLOG(PSICB)
          DLPSIC =DLOG(PSIC)
          SE0=(PSICB/PSIC0)**DLAM
C          TERM1=-ZETA/DLAMP12/DLPSIC0
...
C         TERM1 IS CROSSED OUT IN ALL OF THE GAMAS
     ...
     1        (TERM2-DLAM1P1*DLPSIC)))
        GAM0=SE0/PSIC0*(TERM2+SWRES*(-1.D0+DLAMP12/SE0))
