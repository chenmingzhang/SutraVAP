...
C XX(I) IS DUE TO HEAT EMMISION IS FROM SIDE (YZ PLANE)
...
C-----------BOTTOM CELL ---------------
      ...
      HS2 = +SCF*CA*SATM*DMAIR/RC/TSDK/RHVS(I)/XX(I)   
C  THE XX(I)HEAT EMMISION IS FROM SIDE (YZ PLANE) RATHER THAN TOP 
C (XY PLANE)
      BB(I)=S2+DT2+C2+QV2+QL2+HS2
...
C   ! HEAT EXCHANGE FROM THE TOP
      ...
      DTR = +CL*TIK*DRSNDT(I)                     
      HSR = +SCF*CA*SATM*DMAIR/RC/RHVS(I)/XX(I)   
C!THE XX(I) THE HEAT EMMISION IS FROM SIDE (YZ PLANE) RATHER THAN TOP 
C(XY PLANE)
      HTR = +SWRAT*CA*SATM*DMAIR/RC/RAVT/YY(I)
      HRTR= +HSC/YY(I)                            
CHEAT SOURCE HSC IS ADDED， WARNING, HERE HEAT SOURCE IS CALCULATED 
CEXPLICITELY!
      LHTR= +ACET(1)*VLH*RHOW0/YY(I)               
CLHTR!EVAPORATION INDUCED LATENT HEAT, ACET IS NEGATIVE
...
C     CURRENTLY THE SEQUENCE STARTS UPWARD, THEN RIGHTWARD
C     Mon 03 Nov 2014 08:13:08 AM EST  
      K=0
