...
if f2(6)~=0
  ...
  for j=2:30:f2(1)   %:f2(6):f2(1)
   %% ---plot temperature-----
    ...
    plot([mean(tlab(1:ndt(5),5)),mean(tlab(1:ndt(4),4)),mean(tlab(1:ndt(3),3))],[0.12,0.17,0.2],'or','markersize',10,'markerface','r');hold off
...
plot(ta(2,1:j)+ttc,st9cm(1:j),'g','LineWidth',lw);hold on
plot(ta(2,1:j)+ttc,st13cm(1:j),'b','LineWidth',lw);hold off
xlabel('Time (day)','FontSize',fz,'FontWeight','bold')
...
plot(ttlab(1:ndt(5),5)-ttlab(1,5),tlab(1:ndt(5),5),'k');hold on
plot(ta(2,1:j)+ttc,tt1cm(1:j),'r','LineWidth',lw);hold on
plot(ta(2,1:j)+ttc,tt3cm(1:j),'g','LineWidth',lw);hold on
plot(ta(2,1:j)+ttc,tt4cm(1:j),'b','LineWidth',lw);hold on
