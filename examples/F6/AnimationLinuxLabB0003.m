...
for i=1:f2(2)-1
  ...
  st15cm(i)=mean(a1(5,(2*(f3(5)+1))-5:(2*(f3(5)+1)),i));
end
...
if f2(6)~=0
  ...
  for j=2:30:f2(1)   %:f2(6):f2(1)
   %% ---plot temperature-----
    ...
    plot(a1(13,(f3(5)+2):(2*(f3(5)+1)),j),a1(2,(f3(5)+2):(2*(f3(5)+1)),j),...
        'LineWidth',lw);hold on
    
    plot([mean(tlab(1:ndt(3),3)),mean(tlab(1:ndt(2),2)),mean(tlab(1:ndt(1),1))]...
        ,[0.12,0.17,0.2],'or','markersize',10,'markerface','r');hold off
   ...
   %% ---Saturation output ----------------
    ...
    plot(b1(4,(f3(5)+1):(2*f3(5)),j)*sce,xye(2,(f3(5)+1):(2*f3(5))),'r',...
        qvy1(1,:,j)*sce,fliplr(xye(2,(f3(5)+1):(2*f3(5)))),'b','LineWidth',lw) 
    ...
    axis([f5(12) f5(11) a1(2,(f3(5)+2),j) a1(2,(2*(f3(5)+1)),j)])   
    ax1 = gca;
    set(ax1,'YTickLabel','')
    set(ax1,'FontSize',fl,'FontWeight','bold')
...
plot(ta(2,1:j)+ttc,st15cm(1:j),'c','LineWidth',lw);hold on
