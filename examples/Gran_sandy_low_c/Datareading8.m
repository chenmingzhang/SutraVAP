...
%% -----------calculating the drying front location----------
if f2(10)~=0
    % ef(1,1:SWELE) is the time of the output
    % ef(2,1:SWELE) is the position of the drying front
    ef1=zeros(2,f2(1));  % initialize the size of ef
    ef1(1,:)=et1(1,:);   
    for i=1:f2(1)
      for j=1:f3(5)+1
          if a1(5,2*(f3(5)+1)-j+1,i)>=f2(10)
              ef1(2,i)=a1(2,2*(f3(5)+1)-j+1,i);
              break
          end
      end
    end
...
if f2(10)~=0
    % ef(1,1:SWELE) is the time of the output
    % ef(2,1:SWELE) is the position of the drying front
    ef2=zeros(2,f2(1));  % initialize the size of ef
    ef2(1,:)=et2(1,:);   
    for i=1:f2(1)
      for j=1:f3(5)+1
          if a2(5,2*(f3(5)+1)-j+1,i)>=f2(10)
              ef2(2,i)=a2(2,2*(f3(5)+1)-j+1,i);
              break
          end
      end
    end
...
if f2(10)~=0
    % ef(1,1:SWELE) is the time of the output
    % ef(2,1:SWELE) is the position of the drying front
    ef3=zeros(2,f2(1));  % initialize the size of ef
    ef3(1,:)=et3(1,:);   
    for i=1:f2(1)
      for j=1:f3(5)+1
          if a3(5,2*(f3(5)+1)-j+1,i)>=f2(10)
              ef3(2,i)=a3(2,2*(f3(5)+1)-j+1,i);
              break
          end
      end
    end
...
%  ------salinity --------
    ...
    for i=1:f6(1)
        ...
        swn(i)=str2double(ln1(i,4));
        slab(1:2,1:swn(i),i)=fscanf(fn, '%g %g', [2 swn(i)]);
    end
    end
% concentration    
    if f6(2)~=0
      clab=zeros(2,30,f6(2));
        ...
        cn(i)=str2double(ln2(i,4));
        clab(1:2,1:cn(i),i)=fscanf(fn, '%g %g', [2 cn(i)]);
    end
    end
% temperature
    if f6(3)~=0
      ...
      for i=1:f6(3)                   
        ...
        tn(i)=str2double(ln3(i,4));      % no of data in this dataset
        tlab(1:2,1:tn(i),i)=fscanf(fn, '%g %g', [2 tn(i)]);   % getting the depth (m) and concentration (-) data
...
% ground evaporation
   if f6(5)~=0
    ...
    gun(i)=str2double(ln(4));
    eglab=fscanf(fn, '%g %g', [2 gun(i)]);
   end
...
toc
%% ---------------- do the evaluation on the position of the 
